...
function library:window(props)
    ...
    library:create("ImageLabel", {
        Parent = topbar, 
        Size = dim2(0, 100, 0, 24), -- Adjust 100 if the logo needs to be wider/narrower
        Position = dim2(0, 10, 0.5, 0), -- Shifted left (10px margin)
    ...
    -- AUTOMATED CONFIG SYSTEM CORE
    function win:GetConfigData()
        local data = {}
        for name, el in pairs(self._elementRegistry) do
            if not el.NoSave and type(el.get_value) == "function" then
                data[name] = el:get_value()
            end
        end
        return data
    end
    ...
    function win:LoadConfigData(data)
        ...
        for name, value in pairs(data) do
            ...
            if el and type(el.set) == "function" then
                -- Protected Call: Prevents bad saves from breaking the entire loop
    ...
    function win:Tab(props)
        ...
        function section_api:Toggle(p)
            ...
            -- DATA API
            function tog:get_value() return self.enabled end
            
            function tog:set(val)
                local finalState = self.enabled
                
                -- Crash-Proof Conversion: Safely handle table saves or boolean saves
                if type(val) == "table" and val.State ~= nil then
                    finalState = val.State == true
                ...
                if p.Callback then 
                    task.spawn(function() p.Callback(finalState) end)
                end
            end
        ...
        function section_api:Slider(p)
            ...
            val_box.FocusLost:Connect(function()
                ...
                if num then
                    ...
                    current_val = num
                    fill.Size = dim2((num - min) / (max - min), 0, 1, 0)
                    val_box.Text = string.format("%." .. decimals .. "f", num)
                    if p.Callback then p.Callback(num) end
                else
                    val_box.Text = string.format("%." .. decimals .. "f", current_val)
                end
            end)
        ...
        function section_api:Dropdown(p)
            ...
            searchBox:GetPropertyChangedSignal("Text"):Connect(function() 
                local q = searchBox.Text:lower(); for _, iBtn in pairs(itemBtns) do iBtn.btn.Visible = (q == "" or iBtn.name:lower():find(q) ~= nil) end 
            end)
            ...
            local api = {
                ...
                set = function(self, val)
                    selected = val
                    updateItems()
                    if p.Callback then task.spawn(function() p.Callback(selected) end) end
                end,
            }
            ...
            local api = {
                ...
                set = function(self, new_key)
                    if type(new_key) == "string" then
                        local s, k = pcall(function() return Enum.KeyCode[new_key] end)
                        if s and k then key = k end
                    elseif typeof(new_key) == "EnumItem" then
                        key = new_key
                    end
                    btn.Text = "  " .. (p.Name or p.name or "Keybind") .. " : [" .. key.Name .. "]"
        ...
        function tab:Section(props)
            ...
            -- Create a transparent wrapper to hold both the header and the elements box
            local wrapper = library:create("Frame", { 
                Parent = parent_col, Size = dim2(1, 0, 0, 0), BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y 
            })
            library:create("UIListLayout", {Parent = wrapper, Padding = dim(0, 6), SortOrder = Enum.SortOrder.LayoutOrder})
            
            -- Create the section header (moved OUTSIDE the section box)
            local header_frame = library:create("Frame", {
                Parent = wrapper, Size = dim2(1, 0, 0, 22), BackgroundTransparency = 1, LayoutOrder = 1
            })
            
            -- Use UIListLayout to flawlessly align icon and text horizontally
            library:create("UIListLayout", {
                Parent = header_frame, FillDirection = Enum.FillDirection.Horizontal, 
                Padding = dim(0, 6), VerticalAlignment = Enum.VerticalAlignment.Center,
                HorizontalAlignment = Enum.HorizontalAlignment.Center
            })
            
            -- Load Lucide icon if provided
            local s_icon = props.icon or props.Icon
            if s_icon then
                local icon_obj = get_icon(s_icon, Theme.Accent)
                if icon_obj then
                    icon_obj.Parent = header_frame
                    icon_obj.Size = dim2(0, 18, 0, 18) -- Same relative size as the bigger text
                end
            end
            
            -- Larger section text
            library:create("TextLabel", { 
                Parent = header_frame, Text = props.name or props.Name or "Section", 
                Size = dim2(0, 0, 1, 0), AutomaticSize = Enum.AutomaticSize.X, 
                BackgroundTransparency = 1, TextColor3 = Theme.Accent, 
                FontFace = library.font, TextSize = 16, TextXAlignment = Enum.TextXAlignment.Center 
            })

            -- The actual body of the section (Identical variable structure so scripts won't break)
            s.elements = library:create("Frame", { 
                Parent = wrapper, Size = dim2(1, 0, 0, 0), BackgroundColor3 = Theme.SectionBG, 
            ...
            library:create("UIPadding", {
                Parent = s.elements, PaddingTop = dim(0, 10), PaddingBottom = dim(0, 10), 
                PaddingLeft = dim(0, 10), PaddingRight = dim(0, 10)
            })

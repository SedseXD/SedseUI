...
function library:Unload()
    ...
    if parent:FindFirstChild("MonolithFloats") then parent.MonolithFloats:Destroy() end
...
function library:window(props)
    ...
    library:create("TextLabel", {
        Parent = topbar, Text = (props.name or props.Name or "Monolith UI"),
        Size = dim2(1, -(titleOff + 80), 1, 0), Position = dim2(0, titleOff, 0, 0),
        BackgroundTransparency = 1, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left,
        FontFace = library.font, TextSize = 18
    })
    ...
    -- ── Shared window-scope state (ONE declaration each) ──────────────────
    local all_toggles = {}
    ...
    resizeHandle.InputBegan:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            resizing = true; rStartPos = input.Position; rStartSize = main.Size
        end
    end)
    track_connection(uis.InputChanged:Connect(function(input)
    ...
    minBtn.MouseButton1Click:Connect(function()
        ...
        if isMinimized then
            savedSize = main.Size
            resizeHandle.Visible = false; sidebar.Visible = false; page_holder.Visible = false; topbar_filler.Visible = false
            ...
            t.Completed:Connect(function()
                if not isMinimized then
                    sidebar.Visible = true; page_holder.Visible = true; resizeHandle.Visible = true
                end
            end)
        end
    end)
    ...
    -- Single InputBegan for menu toggle — reads the shared toggleKey upvalue
    ...
    if props.Loading then
        local bootCoroutine = BootSequence(main, props.name or "Monolith UI")
        bootCoroutine()
    end
    ...
    function win:Tab(props)
        ...
        function section_api:Label(p)
            local l = library:create("TextLabel", {
                ...
                Text = p.name or p.Name or "Label", TextColor3 = Theme.MutedText, FontFace = library.font,
                TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextWrapped = true, AutomaticSize = Enum.AutomaticSize.Y
            })
        ...
        function section_api:Button(p)
            local b = library:create("TextButton", {
                ...
                Text = " " .. (p.name or p.Name or "Button"), TextColor3 = Theme.Text, FontFace = library.font,
        ...
        function section_api:Slider(p)
            ...
            library:create("TextLabel", {
                Parent = s, Text = "  " .. (p.name or p.Name or "Slider"), Size = dim2(1, 0, 0, 25),
                BackgroundTransparency = 1, TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left,
                FontFace = library.font, TextSize = 13
            })
            local val_lbl = library:create("TextLabel", {
                Parent = s, Text = string.format("%." .. decimals .. "f", default),
                Size = dim2(0, 50, 0, 25), Position = dim2(1, -55, 0, 0),
                BackgroundTransparency = 1, TextColor3 = Theme.Accent, TextXAlignment = Enum.TextXAlignment.Right,
                FontFace = library.font, TextSize = 13
            })
        ...
        function section_api:Toggle(p)
            ...
            local holder = library:create("Frame", {
                ...
                Size = dim2(1, 0, 0, 0), BackgroundTransparency = 1, AutomaticSize = Enum.AutomaticSize.Y
            })
            library:create("UIListLayout", { Parent = holder, Padding = dim(0, 6) })

            -- Row: [KB] [Toggle Button ─────── ■]
            local row = library:create("Frame", {
                Parent = holder, Size = dim2(1, 0, 0, 32), BackgroundTransparency = 1
            })

            local kbBtn = library:create("TextButton", {
                Parent = row, Size = dim2(0, 32, 1, 0),
                BackgroundColor3 = Theme.ElementBG, Text = "[?]",
                TextColor3 = Theme.MutedText, FontFace = library.font, TextSize = 9, AutoButtonColor = false
            })
            ...
            local btn = library:create("TextButton", {
                Parent = row, Size = dim2(1, -38, 1, 0), Position = dim2(0, 38, 0, 0),
                BackgroundColor3 = Theme.ElementBG, Text = "  " .. (p.name or p.Name or "Toggle"),
                TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left,
                FontFace = library.font, TextSize = 13, AutoButtonColor = false
            })
            library:create("UICorner", { Parent = btn, CornerRadius = dim(0, 6) })
            library:create("UIStroke", { Parent = btn, Color = Theme.Outline, Thickness = 1 })
            ...
            local indicator = library:create("Frame", {
                Parent = btn, Size = dim2(0, 16, 0, 16), Position = dim2(1, -24, 0.5, -8),
            ...
            local container = library:create("Frame", {
                Parent = holder, Size = dim2(1, 0, 0, 0), BackgroundTransparency = 1,
                Visible = tog.enabled, AutomaticSize = Enum.AutomaticSize.Y
            })
        ...
        function section_api:Dropdown(p)
            ...
            local btn = library:create("TextButton", {
                Parent = holder, LayoutOrder = 1, Size = dim2(1, 0, 0, 32), BackgroundColor3 = Theme.ElementBG,
                Text = "  " .. (p.Name or p.name or "Dropdown") .. " : " .. get_val_str(),
                TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, FontFace = library.font, TextSize = 13, AutoButtonColor = false
            })
            library:create("UICorner", {Parent = btn, CornerRadius = dim(0, 6)})
            library:create("UIStroke", {Parent = btn, Color = Theme.Outline, Thickness = 1})
            if p.Premium or p.premium then PremiumOverlay(btn) end
            ...
            if iconDown and iconUp then
                iconDown.Size = dim2(0, 16, 0, 16); iconDown.Position = dim2(1, -26, 0.5, 0); iconDown.AnchorPoint = Vector2.new(0, 0.5); iconDown.Parent = btn; iconDown.Visible = true
                iconUp.Size   = dim2(0, 16, 0, 16); iconUp.Position   = dim2(1, -26, 0.5, 0); iconUp.AnchorPoint   = Vector2.new(0, 0.5); iconUp.Parent   = btn; iconUp.Visible   = false
            end
            ...
            local searchBox = library:create("TextBox", {
                Parent = container, LayoutOrder = 1, Size = dim2(1, 0, 0, 28), BackgroundColor3 = Theme.MainBG,
                TextColor3 = Theme.Text, PlaceholderText = "Search...", PlaceholderColor3 = Theme.MutedText,
                FontFace = library.font, TextSize = 12, TextXAlignment = Enum.TextXAlignment.Left, Text = ""
            })
            ...
            btn.MouseButton1Click:Connect(function()
                ...
                if iconDown and iconUp then iconDown.Visible = not open; iconUp.Visible = open end
            end)
            ...
            local function build_items(itemList)
                ...
                for index, item in pairs(itemList or {}) do
                    local ibtn = library:create("TextButton", {
                        Parent = container, LayoutOrder = index + 1, Size = dim2(1, 0, 0, 26),
                        BackgroundColor3 = Theme.HoverBG, Text = "  " .. item, TextColor3 = Theme.MutedText,
                        TextXAlignment = Enum.TextXAlignment.Left, FontFace = library.font, TextSize = 12, AutoButtonColor = false
                    })
        ...
        function section_api:Colorpicker(p)
            ...
            local btn = library:create("TextButton", {
                ...
                Text = "  " .. (p.Name or p.name or "Colorpicker"), TextColor3 = Theme.Text,
                TextXAlignment = Enum.TextXAlignment.Left, FontFace = library.font, TextSize = 13, AutoButtonColor = false
            })
            library:create("UICorner", {Parent = btn, CornerRadius = dim(0, 6)})
            library:create("UIStroke", {Parent = btn, Color = Theme.Outline, Thickness = 1})
            if p.Premium or p.premium then PremiumOverlay(btn) end
            ...
            local angle0 = (h * math.pi * 2) - (math.pi / 2)
            pickerDot.Position = dim2(0.5 + math.cos(angle0) * 0.5 * s, 0, 0.5 + math.sin(angle0) * 0.5 * s, 0)
            ...
            track_connection(uis.InputChanged:Connect(function(input)
                if input.UserInputType == Enum.UserInputType.MouseMovement or input.UserInputType == Enum.UserInputType.Touch then
                    ...
                    if draggingWheel then
                        ...
                        h = ((ang / (math.pi * 2)) + 0.5) % 1
                        s = offset.Magnitude / rad
                        update_color()
                    elseif draggingVal then
                        local relY = correctedMouse.Y - valSlider.AbsolutePosition.Y
                        local clampY = math.clamp(relY, 0, valSlider.AbsoluteSize.Y)
                        valIndicator.Position = dim2(0.5, 0, 0, clampY)
                        v = 1 - (clampY / valSlider.AbsoluteSize.Y)
                        update_color()
                    end
                end
            end))
            return {
                set = function(self, new_color)
                    ...
                    pickerDot.Position = dim2(0.5 + math.cos(ang) * 0.5 * s, 0, 0.5 + math.sin(ang) * 0.5 * s, 0)
        ...
        function section_api:Keybind(p)
            ...
            local btn = library:create("TextButton", {
                Parent = p.Parent or self.elements, Size = dim2(1, 0, 0, 32), BackgroundColor3 = Theme.ElementBG,
                Text = "  " .. (p.Name or p.name or "Keybind") .. " : [" .. key.Name .. "]",
                TextColor3 = Theme.Text, TextXAlignment = Enum.TextXAlignment.Left, FontFace = library.font, TextSize = 13, AutoButtonColor = false
            })
            library:create("UICorner", {Parent = btn, CornerRadius = dim(0, 6)})
            library:create("UIStroke", {Parent = btn, Color = Theme.Outline, Thickness = 1})
            ...
            function list_api:Add(name)
                ...
                local function swap(dir)
                    ...
                    if idx and list_api.items[idx + dir] then
                        ...
                        if p.Callback then
                            local res = {}
                            for _, v in ipairs(list_api.items) do table.insert(res, v:FindFirstChildOfClass("TextLabel").Text:gsub("^%s+", "")) end
                            p.Callback(res)
                        end
                    end
                end
                up.MouseButton1Click:Connect(function() swap(-1) end)
                dn.MouseButton1Click:Connect(function() swap(1) end)
                return itemBtn
            end
    ...
    -- ══════════════════════════════════════════
            ...
            for _, v in ipairs(screen:GetDescendants()) do
                pcall(function()
                    ...
                    elseif v:IsA("TextLabel") or v:IsA("TextButton") or v:IsA("TextBox") then
                        if colorMatches(v.BackgroundColor3, old) then library:tween(v, { BackgroundColor3 = new_color }, 0.2) end
                        if colorMatches(v.TextColor3, old) then library:tween(v, { TextColor3 = new_color }, 0.2) end
                    elseif v:IsA("ImageLabel") or v:IsA("ImageButton") then
                        if colorMatches(v.BackgroundColor3, old) then library:tween(v, { BackgroundColor3 = new_color }, 0.2) end
                        if colorMatches(v.ImageColor3, old) then library:tween(v, { ImageColor3 = new_color }, 0.2) end
        ...
        colorSection:Colorpicker({ name = "Accent",          default = Theme.Accent,    Callback = function(c) apply_theme("Accent", c) end })
        colorSection:Colorpicker({ name = "Background",      default = Theme.MainBG,    Callback = function(c) apply_theme("MainBG", c) end })
        colorSection:Colorpicker({ name = "Sidebar & Topbar",default = Theme.SidebarBG, Callback = function(c) apply_theme("SidebarBG", c); apply_theme("TopbarBG", c) end })
        colorSection:Colorpicker({ name = "Elements",        default = Theme.ElementBG, Callback = function(c) apply_theme("ElementBG", c) end })
        colorSection:Colorpicker({ name = "Text",            default = Theme.Text,      Callback = function(c) apply_theme("Text", c) end })
        ...
        if #all_toggles == 0 then
            ...
            local float_screen = library:create("ScreenGui", {
                Parent = ui_parent, Name = "MonolithFloats", ResetOnSpawn = false, DisplayOrder = 999
            })
            ...
            local function make_float(entry, idx)
                ...
                local fb = library:create("TextButton", {
                    ...
                    FontFace = library.font, TextSize = 11, AutoButtonColor = false, ZIndex = 5
                })
                library:create("UICorner", { Parent = fb, CornerRadius = dim(0, 22) })
                library:create("UIStroke", { Parent = fb, Color = Theme.Accent, Thickness = 1.5 })
                library:draggify(fb)
            ...
            local toggle_names = {}
            for _, t in ipairs(all_toggles) do table.insert(toggle_names, t.name) end

            mobileSection:Label({ name = "Select toggles to show as floating buttons." })
            mobileSection:Dropdown({
                name = "Floating Buttons", items = toggle_names, multi = true, default = {},
                Callback = function(selected_list)
                    for _, t in ipairs(all_toggles) do
                        if t.float_btn then t.float_btn:Destroy(); t.float_btn = nil end
                    end
                    local idx = 1
                    for _, sel_name in ipairs(selected_list) do
                        for _, t in ipairs(all_toggles) do
                            if t.name == sel_name then make_float(t, idx); idx = idx + 1; break end
                        end
                    end
                end
            })
        ...
        controlSection:Keybind({
            name = "Toggle Menu", default = toggleKey,
            Callback = function(key)
                toggleKey = key  -- writes back to the single shared upvalue
            end
        })
        controlSection:Slider({
            name = "UI Opacity", min = 10, max = 100, default = 100, decimals = 0,
...
function library:create_notification(props)
    ...
    local notif = library:create("Frame", {
        Parent = notif_container, Size = dim2(1, 0, 0, 40),
        BackgroundColor3 = Theme.ElementBG, BackgroundTransparency = 1
    })
    ...
    local title = library:create("TextLabel", {
        Parent = notif, Size = dim2(1, -20, 1, 0), Position = dim2(0, 10, 0, 0),
        BackgroundTransparency = 1, Text = name, TextColor3 = Theme.Text,
        FontFace = library.font, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextTransparency = 1
    })

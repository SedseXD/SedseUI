...
function library:window(props)
    ...
    function win:Tab(props)
        ...
        function tab:Label(props)
            ...
            library:create("UICorner", {Parent = container, CornerRadius = dim(0, 8)})
            library:create("UIStroke", {Parent = container, Color = Theme.Outline, Thickness = 1})
            library:create("UIPadding", {Parent = container, PaddingTop = dim(0, 10), PaddingBottom = dim(0, 10), PaddingLeft = dim(0, 10), PaddingRight = dim(0, 10)})
            
            -- Create the actual text label
            local lbl = library:create("TextLabel", { 
                Parent = container, 
                Text = props.text or props.Text or props.name or props.Name or "Label Text", 
                Size = dim2(1, 0, 0, 0), 
                BackgroundTransparency = 1, 
            ...
            -- Expose an API to easily update the text
            local api = {
                set = function(self, new_text) lbl.Text = tostring(new_text) end,
                set_value = function(self, new_text) lbl.Text = tostring(new_text) end
            }
            
            -- Register to window elements for global updating
            local el_name = props.id or props.name or props.Name
            if el_name then win._elementRegistry[el_name] = api end
